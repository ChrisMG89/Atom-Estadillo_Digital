'''-------------------------------------------------------------------------------
 ...
 Autor:
 Autor Modificacion : Christian Martin Gonzalez      
 Creado:     dd/mm/2021
 Modificado : 26/02/2024
 Copyright:  (c) 2021. All rights reserved. Aerotools-UAV
 Licencia:     
 Interprete: Python 3.12
 
 Versiones:   v1(18/10/2021):
                - Creado un script solo para las funciones
              v2(26/02/2021):
                - Modificacion del color de la interfaz pasando de color "tomate" a color corporativo "#A7A7A7"
                - Modificacion de los ComboBox"Country","PV Enterprise" y "Project" , habilitando state "normal" para que el usuario pueda añadir nuevos datos
                - Generacion de CSVs para almacenar nuevos valores para "Country" , "PV Enterprise" y "Project"
-------------------------------------------------------------------------------'''
...
#pyinstaller --onefile --windowed --icon=ubicacion_icono.ico estadillo_ingles.py
...
raiz.iconbitmap('atom.ico')#Icono de barra de tareas y de aplicacion
...
miFrame.config(bg="#A7A7A7") #cambia el color del frame
...
def guardar():
    ...
    if lista_desplegable_tipo_trabajo.get() == 'Photovoltaic':
        ...
        #------------Crea el DF donde se guarda el estadillo
            ...
            new_row = {'Country':lista_desplegable_country.get(),'Enterprise': lista_desplegable_empresa.get(), 'Project':lista_desplegable_trabajo.get(), 'Date':fecha.get(),'Pilot': lista_desplegable_piloto.get(),'Flight_equipment':lista_desplegable_equipo.get(),
                ...
                'Flight_speed':lista_desplegable_velvuelo.get(),'Flight_height': alt.get(),'Air_speed': lista_desplegable_velaire.get(), 'Air_temp': lista_desplegable_temp.get(), 'Clouds':lista_desplegable_nubes.get(),
                 ...
                 'Initial_Cali': Multy1.get(),'Final_Cali':Multy2.get(),'Typology': lista_desplegable_tipologia.get(),'Flight_aborted': vuelo_abortado.get()}
            ...
            # Cambia punto por coma a voltaje inicial, final y pitch
            df['Initial_volt'] = df['Initial_volt'].str.replace('.', ',')
            df['Final_volt'] = df['Final_volt'].str.replace('.', ',')
            df['Pitch'] = df['Pitch'].str.replace('.', ',')
            ...
            s = pd.DataFrame(columns=['year','planta','powerblock','vuelo','desp_x','desp_y']) # Crear DataFrame s
            new_row_s = {'year': localtime[0], 'planta': (lista_desplegable_trabajo.get()).upper(), 'powerblock': pb.get(), 'vuelo': v.get(), 'desp_x': desplazamiento_x(), 'desp_y': desplazamiento_y()}
            s = pd.concat([s, pd.DataFrame([new_row_s])], ignore_index=True)
...
canvas = Canvas(miFrame, width = 250, height = 90, bg="#A7A7A7", highlightthickness=0)  
...
# Función para cargar valores desde un archivo CSV
def cargar_valores_desde_csv(nombre_archivo):
    ruta = os.path.join("CSV_load", nombre_archivo)
    try:
        with open(ruta, newline='') as archivo_csv:
            reader = csv.reader(archivo_csv)
            return [fila[0] for fila in reader]
    except Exception as e:
        print("Error al cargar valores desde el archivo CSV:", e)
        return []

# Función para cargar los valores desde los archivos CSV
def cargar_valores_desde_archivos():
    # Cargar valores de los archivos CSV
    empresas = cargar_valores_desde_csv('Enterprise.csv')
    trabajos = cargar_valores_desde_csv('Project.csv')
    paises = cargar_valores_desde_csv('Country.csv')

    # Imprimir los valores de cada archivo CSV
    print("Valores de Enterprise.csv:", empresas)
    print("Valores de Project.csv:", trabajos)
    print("Valores de Country.csv:", paises)

    return empresas, trabajos, paises

# Función para actualizar los comboboxes con los valores cargados desde los archivos CSV
def actualizar_comboboxes():
    empresas, trabajos, paises = cargar_valores_desde_archivos()
    lista_desplegable_empresa['values'] = empresas
    lista_desplegable_trabajo['values'] = trabajos
    lista_desplegable_country['values'] = paises

def generar_csv_si_no_existe(nombre_archivo, datos):
    ruta = os.path.join("CSV_load", nombre_archivo)
    if not os.path.exists("CSV_load"):
        os.makedirs("CSV_load")
...
# Función para guardar el valor en el archivo CSV y actualizar el combobox
def guardar_valor_y_generar_csv(event, nombre_archivo, variable, combobox):
    # Obtener el valor del combobox
    valor = variable.get().upper()
    variable.set(valor)
    ...
    print("Valor a guardar:", valor)
    
    # Imprimir la ruta del archivo CSV para verificar
    print("Ruta del archivo CSV:", ruta)

    # Escribir el valor en el archivo CSV
    try:
        with open(ruta, 'a', newline='') as archivo_csv:
            writer = csv.writer(archivo_csv)
            writer.writerow([valor])
            print("Valor guardado correctamente en el archivo CSV.")
    except Exception as e:
        print("Error al escribir en el archivo CSV:", e)

    # Limpiar el combobox después de guardar el valor
    combobox.delete(0, 'end')

      # Actualizar los ComboBox con los nuevos valores
    actualizar_comboboxes()
    

# Crear archivos CSV si no existen
for archivo in ['Enterprise.csv', 'Project.csv', 'Country.csv']:
    generar_csv_si_no_existe(archivo, [])

countryLabel = Label(miFrame, text="Country: ",font=fontStyle)
countryLabel.place(x=0,y=0)
lista_desplegable_country = ttk.Combobox(miFrame, width=12,state='normal',style="custom.TCombobox",font=fontStyle, textvariable=country)
lista_desplegable_country.place(x=120,y=0)
lista_desplegable_country['values']=lista_desplegable_country
#opciones_country = ['Australia','UK','Morocco','Chile','Puerto Rico','Uruguay','Argentina','France','Italy','Romania','USA','Uzbekistan']

empresaLabel = Label(miFrame, text="PV Enterprise: ",font=fontStyle)
empresaLabel.place(x=0,y=70)
lista_desplegable_empresa = ttk.Combobox(miFrame, width=15,font=fontStyle,style="custom.TCombobox",state='normal', textvariable=empresa)
lista_desplegable_empresa.place(x=160,y=70)
#opciones_empresa = ['Ingeteam','METKA','SONNEDIX','COBRA','ABEI','ACCIONA']
lista_desplegable_empresa['values']=lista_desplegable_empresa

trabajoLabel = Label(miFrame, text="Project: ",font=fontStyle)
trabajoLabel.place(x=0, y=110)
lista_desplegable_trabajo = ttk.Combobox(miFrame, width=15,font=fontStyle,style="custom.TCombobox",state='normal', textvariable=trabajo)
lista_desplegable_trabajo.place(x=120,y=110)
#opciones_trabajo = ['Childers','Susan River','Raven Thorpe','Roan Head','Ourzazate','Bougdour','Layoune','Salinas','MOURA','COROWA','JUNEE','WAGGA','WAGGA SOUTH','CALUGARENI','Finisterrae','Santa Julia','El Queule','Santa Rosa','Alto Solar','Chochiñi','Lo Miranda','Malinke','Paraguay','Peralillo','Rinconada','Villa Prat','San Pedro I','San Pedro VI','San Pedro IV','San Pedro III','Azabache','Valle del Sol','Lockma','Mitchi','Ckilir','Doña Carmen']
lista_desplegable_trabajo['values']=lista_desplegable_trabajo

boton_fecha = Button(miFrame, text = "Date", command=fecha_boton,font=fontStyle,bg='#C1C1C1')
...
opciones_equipo = ['AT4-01','AT4-05','AT6','AT6-02','DJI M200','DJI M210','DJI M300','MAVIC 2EA','DJI Mavic 3T','DJI M30T','DJI Matrice 350 RTK']
...
# Vincular la función de conversión a mayúsculas con los Combobox
lista_desplegable_country.bind('<Return>', lambda event: (print("Valor de country:", country.get()), guardar_valor_y_generar_csv(event, 'Country.csv', country, lista_desplegable_country)))
lista_desplegable_empresa.bind('<Return>', lambda event: (print("Valor de empresa:", country.get()), guardar_valor_y_generar_csv(event, 'Enterprise.csv', empresa, lista_desplegable_empresa)))
lista_desplegable_trabajo.bind('<Return>', lambda event: (print("Valor de trabajo:", country.get()), guardar_valor_y_generar_csv(event, 'Project.csv', trabajo, lista_desplegable_trabajo)))

# Función para actualizar los comboboxes
actualizar_comboboxes()
...
boton_print = Button(miFrame, text = "CSV", command=open_csv,font=font_boton,bg='#C1C1C1')
boton_print.place(x=1300,y=680)


boton_hora = Button(miFrame, text = "Initial time", command=hora_boton,font=fontStyle,bg='#C1C1C1')
...
boton_hora_fin = Button(miFrame, text = "Final time", command=hora_boton_fin,font=fontStyle,bg='#C1C1C1')
...
lista_desplegable_velvuelo = ttk.Combobox(miFrame, width=4,state='normal',style="custom.TCombobox",font=fontStyle)
...
# Función para validar la entrada en el Combobox
def validar_entrada(event):
    # Obtener el carácter ingresado
    caracter = event.char

    # Verificar si el carácter ingresado es un número o un punto
    if not (caracter.isdigit() or caracter == '.' or caracter == '\x08'):  # '\x08' es el código ASCII para la tecla 'backspace'
        return 'break'  # Evita que el carácter se agregue al Combobox

# Configurar la validación en el evento <Key>
lista_desplegable_velvuelo.bind('<Key>', validar_entrada)
...
boton_guardar=Button(miFrame, text = "Save", command=guardar,width=10,height=1,font=font_boton,bg='#C1C1C1')
